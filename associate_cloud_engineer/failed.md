...
組織のセキュリティ チームは、Cloud SQL データベースの操作に関連する潜在的なセキュリティインシデントを調査するために、プロジェクトの Cloud Audit Logs へのアクセスをリクエストしましが、データベースの作成と削除以外のログを表示することができません。詳細なデータベースアクセスログを表示するために必要な権限をセキュリティチームに付与するにはどうすればよいでしょうか?

> [!TIP]
> クラウド SQL サービスのデータアクセスログを有効にし、セキュリティチームにプライベートログ閲覧者のロールを付与する。

> [!NOTE]
> データアクセスログは、デフォルトのアクティビティログには含まれない読み取りおよび書き込み操作を含む、リソース上で実行された操作に関する詳細な情報を提供します。セキュリティチームがこれらの詳細なログを表示できるようにするには、Cloud SQL サービスのデータアクセスログを有効にする必要があります。これらのログを表示するには、ログ内のプライベートデータにアクセスするために必要な権限を付与するプライベートログ閲覧者のロールが必要です。

> [!WARNING]
> Cloud Logging API を有効にし、セキュリティチームにログビューアロールを付与する。
>
> Cloud Logging API を有効にし、ログビューアロールを付与することで、ユーザーはログを閲覧できるようになりますが、このロールにはデータアクセスログを閲覧する権限は含まれていません。ログ・ビューワの役割は、一般的に標準的なログを閲覧するためのものであり、詳細なアクセス・ログを閲覧するためのものではありません。

## Cloud SQL vs Cloud Spanner

あなたは、グローバルに使用される金融取引アプリケーションを開発しています。データはリレーショナル構造を使用して保存され、世界中のクライアントがまったく同じ状態のデータを取得できる必要があります。アプリケーションは複数のリージョンにデプロイされ、エンドユーザーに最小限の遅延を保証します。待ち時間を最小限に抑えながら、アプリケーションデータのストレージオプションを選択する必要があります。以下の内、最適なものを選択して下さい。

> [!TIP]
> データストレージに Cloud Spanner を使用する。

> [!NOTE]
> Cloud Spanner は、フルマネージドで、水平方向にスケーラブル、グローバルに分散され、強力な一貫性を備えたデータベースサービスです。低レイテンシのアクセスを提供するように設計されており、SQL クエリによるリレーショナルデータ構造をサポートします。Cloud Spanner のグローバル分散機能により、複数のリージョン間でデータが同期的に複製され、世界中のクライアントにまったく同じ状態のデータが提供されます。そのため、リアルタイムの精度と一貫性が求められる金融取引アプリケーションに最適です。

> [!WARNING]
> Cloud SQL は、MySQL、PostgreSQL、SQL Server をサポートするフルマネージドのリレーショナルデータベースサービスです。ただし、グローバル分散向けに設計されておらず、Cloud Spanner と同じレベルのスケーラビリティと低レイテンシは提供していません。Cloud SQL は、グローバルアプリケーションよりもリージョンデプロイメントに適しています。

## データストレージソリューションの設計

あなたは、グローバルなソーシャルメディア・アプリケーションのリアルタイム分析アプリケーションを開発しています。このプラットフォームは、大量のユーザーインタラクションを処理し、データを効率的に永続化する必要があります。ビジネスチームは、ユーザーエンゲージメントを分析するために複雑な SQL クエリを実行する能力を必要とします。データストレージソリューションをどのように設計すべきでしょうか？

> [!TIP]
> 最適化されたスキーマを使用してマルチリージョンの Cloud Spanner インスタンスをデプロイする。

> [!NOTE]
> Cloud Spanner は、グローバルなスケーラビリティと強力な一貫性を実現するために設計されており、SQL クエリをビルトインでサポートしています。大量のトランザクションを効率的に処理するために必要な機能を提供し、複雑な SQL クエリをサポートしているため、このシナリオに最適です。

> [!WARNING]
> リアルタイム書き込みには Bigtable を使用し、分析には別の BigQuery データセットを使用する。
>
> Bigtable は、高スループットかつ低レイテンシのリアルタイム書き込みに優れていますが、SQL クエリをネイティブにサポートしていません。分析に BigQuery を使用すると、システム間でデータを移動することになり、リアルタイム分析では効率が悪くなる可能性があります。

## Google Workspace について

あなたが管理する Google Cloud 環境の一部のプロジェクトに、組織の Google Workspace ドメインに属さないメールアドレスを持つ外部ユーザーが存在していることがわかりました。今後は、ドメインの電子メール アドレスを持つユーザーのみがプロジェクトにアクセスできるようにしたいと考えています。手動による介入を最小限にしてポリシーを適用するには、どうすればよいでしょうか?

> [!TIP]
> 組織ポリシーの制約を設定してドメイン内のユーザーへのアクセスを制限し、既存の外部ユーザーを削除するスクリプトを作成する。
>
> このオプションは、問題の予防と修復の両方の側面に対処します。組織のポリシー制約を設定すると、ドメイン内のユーザーのみが今後プロジェクトにアクセスできるようになります。また、既存の外部ユーザーを削除するスクリプトを作成すると、現在の状態をクリーンアップするのに役立ちます。このアプローチは、自動化されたポリシーの適用と初期のクリーンアッププロセスを組み合わせることで、継続的な手動での介入を最小限に抑えた包括的なソリューションを提供します。

> [!WARNING]
> Cloud Identity を使用してドメイン制限を適用し、各プロジェクトのアクセス制御を手動で監査および更新する。
